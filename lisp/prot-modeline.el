...
(defun find-line-in-buffer (pattern)
  "Find a line containing PATTERN in the current buffer."
  ...
  (let ((found nil))
    (while (and (not found)
      ...
      (let ((line-start (line-beginning-position))
        ...
        (setq found (buffer-substring-no-properties line-start line-end))))
    found))

(defun find-line-in-file (filename pattern)
  "Find a line containing PATTERN in the file FILENAME."
  (with-temp-buffer
    (insert-file-contents filename)
    (find-line-in-buffer pattern)))
...
`default'."
  (propertize
   ...
   'face (if (mode-line-window-selected-p) (or face 'mode-line) 'shadow)
...
`prot-modeline-evil-state-tags'."
  (pcase evil-state
    ...
    ('insert (prot-modeline--evil-propertize-tag 'insert variant 'prot-modeline-indicator-green))
...
(defvar-local prot-modeline-evil
    '(:eval
      (if (and (bound-and-true-p evil-mode))
          (prot-modeline-evil-state-tag :short)
        " "))
  "Mode line construct to display the Evil state.")

;;;; Kube context

(defun prot-modeline--kube-context ()
  ""
  (cadr (split-string (find-line-in-file "~/.kube/config" "current-context") " ")))

(defun prot-modeline--propertize-kube-context (&optional face)
  "Propertize STATE tag of VARIANT with optional FACE.
VARIANT of the state tag is either :short or :long, as defined in
`prot-modeline-evil-state-tags'.  If FACE is nil, fall back to
`default'."
  (concat
   ...
   (propertize
    ...
    'mouse-face 'mode-line-highlight
    'help-echo "Kube Context")))

(defvar-local prot-modeline-kube-context
    '(:eval
      (if (and (mode-line-window-selected-p) (derived-mode-p 'vterm-mode))
          (prot-modeline--propertize-kube-context 'prot-modeline-indicator-cyan)
        ""))
  "Mode line construct to display the current kubernetes context.")
